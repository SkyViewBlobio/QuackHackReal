...
public class CrystalAttackModule extends Module {
    ...
    private BlockPos findBestPlacement(EntityPlayer target, double maxSelfDamage) {
        ...
        // Search in 3D space around target (-4 to +4 in all axes)
        for (int x = -4; x <= 4; x++) {
            for (int y = -2; y <= 2; y++) { // Allow vertical search
                for (int z = -4; z <= 4; z++) {
                    mutablePos.setPos(center.getX() + x, center.getY() + y, center.getZ() + z);

                    if (!isValidPlacement(mutablePos)) continue;

                    // Attacker distance check
                    ...
                    // Line of sight to target's feet
                    ...
                    if (targetDamage < minimumDamagePlace || selfDamage > maxSelfDamage) continue;
    ...
    private double calculateCrystalDamage(Entity target, BlockPos crystalPos) {
        ...
        if (distanceSq > 144.0) return 0.0; // 12^2

        double scaledDistance = Math.sqrt(distanceSq) / 12.0;
        double damage = (1.0 - scaledDistance) * getExplosionPower();

        if (hasObstruction(explosionVec, target.getPositionVector())) damage *= 0.5;
        if (target instanceof EntityPlayer) {
            damage = applyProtectionReductions((EntityPlayer) target, damage);
        }
        if (target == mc.player) {
            damage = applyProximityMultiplier(crystalPos, damage);
        }
    ...
    private double applyProximityMultiplier(BlockPos crystalPos, double damage) {
